    ...
    */
	...
	protected bool t進行描画_チップ(EInstrumentPad ePlayMode, int nPlayer) {
		...
		foreach (var pChip in dTX.listBarLineChip) {
			...
			if (!isInDelay[(int)pChip.nBranch] && (time < 0) && (pChip.nDefOrder > this.lastNoteOrder[nPlayer]))
				this.lastNoteOrder[nPlayer] = pChip.nDefOrder;

			bool forceNMScroll = (pChip.eScrollMode != EScrollMode.Normal) && true /* Jiro1 */ && (pChip.nDefOrder <= this.lastNoteOrder[nPlayer]
				// TaikoJiro 1 behavior: Measures defined non-before but occur before the next BPM Changes are forced to NMScroll
				|| (play_bpm_points[(int)pChip.nBranch].nextBpmChangeAtDiv != null
					&& (pChip.bpmPoint!.n内部番号 >= play_bpm_points[(int)pChip.nBranch].nextBpmChangeAtDiv!.n内部番号
						&& pChip.measureStart!.n発声時刻ms < (int)play_bpm_points[(int)pChip.nBranch].nextBpmChangeAtDiv!.bpm_change_time)));
		...
		foreach (CChip pChip in dTX.listNoteChip) {
			...
			bool forceNMScroll = (velocityRefChip.eScrollMode != EScrollMode.Normal) && true /* Jiro1 */ && (pChip.nDefOrder <= this.lastNoteOrder[nPlayer]
				// TaikoJiro 1 behavior: Measures defined non-before but occur before the next BPM Changes are forced to NMScroll
				|| (play_bpm_points[(int)pChip.nBranch].nextBpmChangeAtDiv != null
					&& (pChip.bpmPoint!.n内部番号 >= play_bpm_points[(int)pChip.nBranch].nextBpmChangeAtDiv!.n内部番号
						&& pChip.measureStart!.n発声時刻ms < (int)play_bpm_points[(int)pChip.nBranch].nextBpmChangeAtDiv!.bpm_change_time)));
