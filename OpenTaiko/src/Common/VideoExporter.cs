...
internal static class VideoExporter {
	...
	public static bool Begin(CommandLineArgs cli) {
		...
		foreach (var err in new[] { cli.DifficultiesError, cli.ClipVolumeError }) {
			if (err != null) {
	...
	private static float[] DecodeToStereoResampled(string path, double msSeek, int sampleRate) {
	...
	private static float[] MixAudio(double durationSec, int samplingRate) {
		...
		foreach (var ev in events) {
			...
			float[] pcm = DecodeToStereoResampled(ev.Path, ev.msSeek, samplingRate);
