...
internal abstract class CStage演奏画面共通 : CStage {
	...
	protected (CChip? chip, ENoteJudge rawJudge) GetChipToJudge(long msTjaTime, int nPlayer, EPad pad) {
		...
		if (chipRoll != null && (
			chip == null || judge is ENoteJudge.Miss || (judge is ENoteJudge.Poor && !(NotesManager.IsADLIB(chip) || NotesManager.IsMine(chip)))
	...
	protected (CChip? chip, ENoteJudge rawJudge) GetChipToJudgeIgnoringRollBody(long msTjaTime, int nPlayer, EPad pad) {
		...
		for (int i = iFutureFirst; i-- > 0;) { // exclude past from future
			...
			if (!chip.IsMissed && !chip.bHit && this.IsInputPadConsumedByChip(nPlayer, chip, pad, msTjaTime, judge)) {
				if (NotesManager.IsGenericRoll(chip)) {
					if (pastFirstUnhit.chip == null || chip.n整数値_内部番号 < pastFirstUnhit.chip.n整数値_内部番号)
		...
		if (pastFirstUnhitRoll.chip != null && (pastFirstUnhit.chip == null || pastFirstUnhitRoll.chip.n整数値_内部番号 < pastFirstUnhit.chip.n整数値_内部番号))
		...
		if (NotesManager.IsMissableNote(pastFirstUnhit.chip))
