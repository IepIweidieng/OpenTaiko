...
internal class CAct演奏Combo音声 : CActivity {
	...
	public override void Activate() {
		...
		for (int i = 0; i < OpenTaiko.ConfigIni.nPlayerCount; i++) {
			...
			var currentDir = ($@"{OpenTaiko.strEXEのあるフォルダ}Global{Path.DirectorySeparatorChar}Characters{Path.DirectorySeparatorChar}{OpenTaiko.SkinG.Characters_DirName[_charaId]}{Path.DirectorySeparatorChar}Sounds{Path.DirectorySeparatorChar}Combo{Path.DirectorySeparatorChar}");
			...
			currentDir = ($@"{OpenTaiko.strEXEのあるフォルダ}Global\Characters\{OpenTaiko.SkinG.Characters_DirName[_charaId]}\Sounds\Tower_Combo\");
			if (Directory.Exists(currentDir)) {
				foreach (var item in Directory.GetFiles(currentDir)) {
					var comboVoice = new CComboVoice();
					comboVoice.bFileFound = true;
