...
internal abstract class CStage演奏画面共通 : CStage {
    ...
    */
	...
	protected bool t進行描画_チップ(EInstrumentPad ePlayMode, int nPlayer) {
		...
		// forward phase, for correct order of non-note events
		for (int nCurrentTopChip = 0; nCurrentTopChip < dTX.listChip.Count; ++nCurrentTopChip) {
...
//#endif
		...
		// backward phase, for displaying notes
	...
	public void t演奏位置の変更(int nStartBar, int nPlayer) {
		...
		for (int i = this.nCurrentTopChip; i >= 0; i--) {
			CChip pChip = dTX.listChip[i];
			int nDuration = (int)CTja.TjaTimeToGameTime(pChip.GetDuration());
