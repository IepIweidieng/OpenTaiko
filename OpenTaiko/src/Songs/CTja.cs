...
internal class CTja : CActivity {
	...
	private double dbLastDivBPM = 120.0;
	...
	public bool isAfterLastBpmPoint = false;
	public bool isBpmChangeInsertedBeforeDiv = false;
	public CChip?[] lastMeasureStart = new CChip?[3];
...
//		public bool bBASS演奏で直前のBASSを消音する;
	...
	public void tProcessAllText(string str全入力文字列, int nBGMAdjust, int Difficulty) {
		if (!string.IsNullOrEmpty(str全入力文字列)) {
			...
			if (!this.bHeaderOnly) {
				...
				if (true /* Jiro1 */) {
					// Fix init BPM if overlapped by other BPM segments
					CBPM bpmPointInit = CStage演奏画面共通.GetNowPBPMPoint(this, this.listBPM[0].bpm_change_time, this.listBPM[0].bpm_change_course, useLastDelay: true).nowBpmPoint;
					double th16_beat_init = CStage演奏画面共通.GetNowPBMTime(bpmPointInit, this.listBPM[0].bpm_change_time);
					this.listBPM[0].bpm_change_bmscroll_time = th16_beat_init;
				}
				...
				foreach (CChip chip in this.listChip) {
					...
					double th16_beat = CStage演奏画面共通.GetNowPBMTime(bpmPoint, chip.db発声時刻ms);
						...
						case 0xE2: { // #JPOSSCROLL
								...
								if (lastJPosScroll == null) {
									...
									if (lastJPosScroll.msMoveDt > 0) {
										...
										double msCanMove = double.Max(0, chip.db発声時刻ms - lastJPosScroll.chip!.db発声時刻ms);
		...
		for (int n = 0; n < input.Length; n++) {
			if (nMode == 0) {
				...
				if (!string.IsNullOrEmpty(input[n]) && NotesManager.FastFlankedParsing(input[n])) {
					...
					if (input[n].StartsWith("#BRANCHSTART")
						|| input[n].StartsWith("#BRANCHEND")
						|| input[n].StartsWith("#NEXTSONG")
						|| input[n] == "#N" || input[n] == "#E" || input[n] == "#M") {
						sb.Append(input[n] + "\n");
					}
	...
	private void t1小節の文字数をカウントしてリストに追加する(string InputText) {
		...
		bool isBranchEnd = InputText.StartsWith("#BRANCHSTART")
			|| InputText.StartsWith("#BRANCHEND")
			|| InputText.StartsWith("#NEXTSONG");
		ECourse? branch = InputText.StartsWith("#M") ? ECourse.eMaster
			: InputText.StartsWith("#E") ? ECourse.eExpert
			: InputText.StartsWith("#N") ? ECourse.eNormal
			: null;
		if (isBranchEnd || branch != null) {
			if (this.n現在の小節数 > this.nLineCountMax)
				this.nLineCountMax = this.n現在の小節数;
			if (isBranchEnd)
				this.nLineCountTemp = this.nLineCountMax;
			this.nLineCountCourseTemp = branch ?? ECourse.eNormal;
		...
		line.n文字数 = InputText.Count(ch => !char.IsWhiteSpace(ch)) - 1;
	...
	private void t命令を挿入する(string InputText) {
		...
		if (command == "#START") {
			...
			chip.db発声時刻ms = (int)(this.dbNowTime + 1000); //2016.07.16 kairera0467 終了時から1秒後に設置するよう変更。
														 // チップを配置。
			...
			if (true /* Jiro1 */) {
				var timeTrunced = (int)(this.dbNowTime * (720.0 / 512)) / (720.0 / 512); // tentative approximant
				this.dbNowBMScollTime += (timeTrunced - this.dbNowTime) * this.dbNowBPM / 15000.0;
				this.dbNowTime = timeTrunced;
				this.isAfterLastBpmPoint = true;
			}

			this.SetBPMPointAtDefCursor(dbBPM, this.n現在のコース, EBPMPointType.Bpm);
			...
			this.SetBPMPointAtDefCursor(this.dbNowBPM, this.n現在のコース, EBPMPointType.Scroll);
			...
			try {
				...
				if (true /* Jiro 1 */) {
					dbLength[1] = Convert.ToDouble(strArray[1].Split('.')[0]); // parsed as an int
					if (dbLength[1] <= 0)
						dbLength[1] += 4294967296.0; // convert to unsigned int
				} else {
			...
			this.SetBPMPointAtDefCursor(this.dbNowBPM, this.n現在のコース, EBPMPointType.Measure);
			...
			if (nDELAY > 0) {
				if (true /* !(TJAP3/OOS) */) {
					this.SetBPMPointAtDefCursor(this.dbNowBPM, this.n現在のコース, EBPMPointType.Delay);
				} else {
					this.dbNowBMScollTime += nDELAY * this.dbNowBPM / 15000;
				}
				this.dbNowTime += nDELAY;
				if (true /* !(TJAP3/OOS) */) {
					this.SetBPMPointAtDefCursor(this.dbNowBPM, this.n現在のコース, EBPMPointType.AfterDelay);
				}
			} else if (nDELAY < 0) {
				if (true /* !(TJAP3/OOS) */) {
					this.SetBPMPointAtDefCursor(this.dbNowBPM, this.n現在のコース, EBPMPointType.AfterDelay);
					this.dbNowBMScollTime += nDELAY * this.dbLastDivBPM / 15000;
				}
				this.dbNowTime += nDELAY;
			}

			this.isAfterLastBpmPoint = true; // For a negative delay
			...
			chip.db発声時刻ms = JudgeChipTime.msTime;
	...
	private void SetBPMPointAtDefCursor(double dbBPM, ECourse branch, EBPMPointType pointType) {
		// deduplicate BPM points
		if (this.n内部番号BPM1to - 2 > 0) {
			var lastBPMPoint = this.listBPM[this.n内部番号BPM1to - 2];
			if (branch == lastBPMPoint.bpm_change_course
				&& pointType != EBPMPointType.Delay && lastBPMPoint.point_type != EBPMPointType.Delay
				&& !this.isAfterLastBpmPoint
				) {
				--this.n内部番号BPM1to; // update last BPM point
				CBPM lastBpmPoint = this.listBPM[this.n内部番号BPM1to - 1];
				if (pointType < lastBpmPoint.point_type) {
					pointType = lastBpmPoint.point_type;
					this.dbNowTime = lastBpmPoint.bpm_change_time;
					this.dbNowBMScollTime = lastBpmPoint.bpm_change_bmscroll_time;
				}
			}
		}
		this.listBPM[this.n内部番号BPM1to - 1] = new CBPM() {
			...
			time_signness = (this.fNow_Measure_m != 0 && dbBPM != 0) ? Math.Sign((this.fNow_Measure_s / this.fNow_Measure_m) / dbBPM)
				: (this.fNow_Measure_m != 0) ? Math.Sign(this.fNow_Measure_s / this.fNow_Measure_m)
				: (dbBPM != 0) ? Math.Sign(this.fNow_Measure_s / dbBPM)
	...
	private void ParseArgCamEndCommand(string command, string argument, int channelNo, ref CChip? camChip, string commandStart) {
		if (camChip != null) {
			...
			var msDiff = (int)(chip.db発声時刻ms - camChip.db発声時刻ms);
	...
	private void ParseArgObjEndCommand(string command, string argument, int channelNo, string animationKey, string commandStart) {
		...
		if (currentObjAnimations.ContainsKey($"{animationKey}_{name}")) {
			...
			var msDiff = (int)(chip.db発声時刻ms - startChip.db発声時刻ms);
	...
	private void InitializeChartDefinitionBody() {
		...
		CBPM bpmInit = new() {
			...
			dbBPM値 = this.BASEBPM,
			time_signness = this.BASEBPM < 0 ? -1 : 1,
			bpm_change_time = this.dbNowTime,
			bpm_change_bmscroll_time = this.dbNowBMScollTime,
			bpm_change_scroll = this.dbNowScroll,
			bpm_change_scroll_y = this.dbNowScrollY,
		...
		this.isAfterLastBpmPoint = false;
		this.dbLastDivBPM = this.BASEBPM;
	...
	private void GotoBranchEnd() {
		...
		for (int i = 0; i < 3; ++i) {
			this.SetBPMPointAtDefCursor(this.dbNowBPM, (ECourse)i, EBPMPointType.BranchEnd);
		}
		#endregion
	...
	private (CChip? chip, double msTime) GetBranchJudgeChipTime(bool delayForRoll) {
		...
		for (int i = this.listChip.Count; i-- > 0;) {
			...
			switch (chip.nChannelNo) {
				...
				case 0x9B: // `#NEXTSONG`, cannot judge earlier
					for (int ib = 0; ib < 3; ++ib)
						judgeChipTimes[ib] ??= (chip, chip.db発声時刻ms + msDanNextSongDelay);
					i = 0; // end searching
					continue;
				case 0x50: // real bar line
					judgeChipTimes[(int)chip.nBranch] ??= (chip, chip.db発声時刻ms);
		...
		if (delayForRoll) {
			var lastRollEnd = lastRollEnds.Where(x => x != null).MaxBy(x => x!.db発声時刻ms);
			if (lastRollEnd != null && lastRollEnd.db発声時刻ms > judgeChipTime.Value.msTime)
				judgeChipTime = (lastRollEnd, lastRollEnd.db発声時刻ms); // judge at end of last roll
		}
			...
			if (InputText.StartsWith("#")) {
				...
				if (this.b小節線を挿入している == false) {
					...
					for (int i = 0; i < (IsEndedBranching == true ? 3 : 1); i++) {
						...
						chip.n整数値 = this.listNoteChip.Count - 1;
						...
						chip.measureStart = this.lastMeasureStart[this.IsEndedBranching ? i : (int)this.n現在のコース] = chip;
					}
				...
				for (int n = 0; n < InputText.Length; n++) {
					...
					if (InputText.Substring(n, 1) == ",") {
						if (n文字数 == 0) {
							if (this.isBpmChangeInsertedBeforeDiv) {
								for (int i = 0; i < (this.IsEndedBranching ? 3 : 1); ++i)
									this.SetBPMPointAtDefCursor(this.dbNowBPM, this.IsEndedBranching ? (ECourse)i : this.n現在のコース, EBPMPointType.BpmAtDiv);
								this.isBpmChangeInsertedBeforeDiv = false;
								this.dbLastDivBPM = this.dbNowBPM;
							}
							this.dbLastTime = this.dbNowTime;
							this.dbLastBMScrollTime = this.dbNowBMScollTime;
							this.dbNowTime += (15000.0 / this.dbNowBPM * ((double)this.fNow_Measure_s / this.fNow_Measure_m) * (16.0 / 1));
							this.dbNowBMScollTime += (((double)this.fNow_Measure_s / this.fNow_Measure_m) * (16.0 / 1));
							this.isAfterLastBpmPoint = true;
						}
						this.n現在の小節数++;
					...
					this.dbLastTime = this.dbNowTime;
					this.dbLastBMScrollTime = this.dbNowBMScollTime;
					this.dbNowTime += (15000.0 / this.dbNowBPM * ((double)this.fNow_Measure_s / this.fNow_Measure_m) * (16.0 / n文字数));
					this.dbNowBMScollTime += (((double)this.fNow_Measure_s / this.fNow_Measure_m) * (16.0 / (double)n文字数));
					this.isAfterLastBpmPoint = true;
				}
			}
		}
	...
	private CChip NewScrolledChipAtDefCursor(int channelNo, int iDiv, int divsPerMeasure, ECourse branch) {
		...
		chip.nBranch = branch;
		chip.measureStart = this.lastMeasureStart[(int)branch];
