...
internal class CTja : CActivity {
	...
	public class CBPM {
		...
		public bool forceNMScroll; // TaikoJiro 1 behavior
		public int n内部番号;
		public int n表記上の番号;
		...
		public override string ToString() {
			...
			builder.Append($"({this.point_type}), BPM:{this.dbBPM値}, ");
...
//		public bool bBASS演奏で直前のBASSを消音する;
		...
		if (!string.IsNullOrEmpty(str全入力文字列)) {
			...
			if (!this.bHeaderOnly) {
				...
				foreach (CChip chip in this.listChip) {
					...
					CBPM bpmPoint = CStage演奏画面共通.GetNowPBPMPoint(this, chip.db発声時刻ms, chip.nBranch).nowBpmPoint;
					double bpm = bpmPoint.dbBPM値;
	...
	private void t命令を挿入する(string InputText) {
		...
		if (command == "#START") {
			...
			if (true /* Jiro1 */) {
				var timeTrunced = (int)(Math.Round(this.dbNowTime * 2, MidpointRounding.AwayFromZero) / 2); // tentative approximant
	...
	private void SetBPMPointAtDefCursor(double dbBPM, ECourse branch, EBPMPointType pointType) {
		...
		if (this.n内部番号BPM1to - 2 > 0) {
			...
			if (pointType != EBPMPointType.Delay && lastBPMPoint.point_type != EBPMPointType.Delay
	...
	private void GotoBranchEnd() {
		...
		for (int i = 0; i < 3; ++i) {
			this.SetBPMPointAtDefCursor(this.dbNowBPM, (ECourse)i, EBPMPointType.Measure);
		}
		#endregion
