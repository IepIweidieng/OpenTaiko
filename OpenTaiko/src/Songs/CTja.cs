...
internal class CTja : CActivity {
	...
	public class CBranchPointInfo {
		...
		public double dbBPM, dbBPMLast, dbBPMUntilLastDiv;
				...
				if (this.COMPAT is ETjaCompat.Jiro1) {
					for (int ib = 0; ib < 3; ++ib) {
						...
						for (CBPM? bpmChange = this.listBPM[ib]; (bpmChange = bpmChange.next_bpm_change) != null;) {
							...
							double dPxJiro1Hs1 = pxTh16Beats_Jiro1NoteDist2 * (bpmChange.bpm_change_bmscroll_time - lastBpmChange.bpm_change_bmscroll_time);
							bpmChange.th16BeatDriftX = (bpmChange.bpm_change_scroll == 0) ?
								lastBpmChange.th16BeatDriftX
								: lastBpmChange.th16BeatDriftX - bpmChange.bpm_change_scroll * dPxJiro1Hs1 % 1 / bpmChange.bpm_change_scroll / pxTh16Beats_Jiro1NoteDist2;
							bpmChange.th16BeatDriftY = (bpmChange.bpm_change_scroll_y == 0) ?
								lastBpmChange.th16BeatDriftY
								: lastBpmChange.th16BeatDriftY - bpmChange.bpm_change_scroll_y * dPxJiro1Hs1 % 1 / bpmChange.bpm_change_scroll_y / pxTh16Beats_Jiro1NoteDist2;

							int ibcNext = bpmChange.next_bpm_change?.nInternalNumber ?? this.listBPM.Count;
							for (int ibp = bpmChange.nInternalNumber + 1; ibp < ibcNext; ++ibp) {
								CBPM bpmPoint = this.listBPM[ibp];
								if (bpmPoint.bpm_change_course != (ECourse)ib)
									continue;
								bpmPoint.th16BeatDriftX = bpmChange.th16BeatDriftX;
								bpmPoint.th16BeatDriftY = bpmChange.th16BeatDriftY;
							}
	...
	private void ParseCommand(string command, string argument, string argumentFull) {
		if (command == "#START") {
			...
			chip.dbSoundTimems = (int)JudgeChipTime.msTime;
	...
	private void InitializeChartDefinitionBody() {
		...
		this.th16LastBpmChangeBeat = this.dbNowBMScrollTime;
