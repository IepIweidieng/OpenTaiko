...
internal abstract class CStage演奏画面共通 : CStage {
	...
	private static void CountNijiScoreNotes(List<CChip> listChip, out int nNotes, out int nBalloonHits, out double msRollTime, int startIdx = 0, int endIdx = -1) {
		...
		for (int i = startIdx; i < endIdx; ++i) {
			...
			if (NotesManager.IsGenericBalloon(_chip)) {
				...
				nBalloonHits += Math.Min(_chip.nBalloon, _expectedHits);
			}

			if (NotesManager.IsRoll(_chip) || NotesManager.IsFuzeRoll(_chip))
				msRollTime += (_chip.end.n発声時刻ms - _chip.n発声時刻ms) / 1000.0;
		}
	}
	...
	public int GetCeilingNijiScore(int player)
		=> Math.Max(1000000,
			...
			+ (int)(Math.Ceiling(this.nRollTimeMs[player] * 16.6 / 10) * 100 * 10));
	...
	public static double GetAddScoreNiji(int nSongNotes, int nSongBalloonHits, double msSongRollTime) {
		...
		return (double)Math.Ceiling((decimal)(
			1000000 - (nSongBalloonHits * 100) - (msSongRollTime * 100 * 16.6)
		) / nSongNotes / 10) * 10;
	}
