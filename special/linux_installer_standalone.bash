...
pinned_tag=$pinned_tag  # replaced with the real release tag in GitHub Action
...
if [ -n "$pinned_tag" ] then
    ...
    version_tag=$(curl -s "https://api.github.com/repos/$git_repo/releases/latest" | jq -r '.tag_name')
...
cached_file="$cache_dir/$version_tag-$archive_filename"
if [ -f "$cached_file" ]; then
    echo "Using cached version: $cached_file"
    cp "$cached_file" "$archive_filename"
else
    # Download using curl with redirect support
    download_url="https://github.com/$git_repo/releases/download/$version_tag/$archive_filename"
