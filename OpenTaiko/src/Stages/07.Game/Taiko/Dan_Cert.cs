	...
	private void UpdateReachStatus(int iSong, int iExam, DanExamScore score) {
		...
		switch (dan_C.ExamType) {
			...
			case Exam.Type.Accuracy:
				...
				if (dan_C.ExamRange != Exam.Range.Less) {
					if (judgeFailure && accPoint >= accPointBetterSuccess) {
						dan_C.ReachStatus = Exam.ReachStatus.Better_Success;
						return;
					}
					if (accPoint >= accPointSuccess) {
						// use its own success gauge rule
						if (accPoint - accPointSuccess < (accPointBetterSuccess - accPointSuccess) / 3.0) {
							dan_C.ReachStatus = Exam.ReachStatus.Success_Or_Better; // reset danger status
						} else if (accPoint - accPointSuccess < 2 * (accPointBetterSuccess - accPointSuccess) / 3.0) {
							dan_C.ReachStatus = Exam.ReachStatus.Near_Better_Success;
						} else {
							dan_C.ReachStatus = Exam.ReachStatus.Nearer_Better_Success;
						}
						return;
					} else if (dan_C.GetExamStatus() < Exam.Status.Success) {
		...
		dan_C.ReachStatus = (dan_C.ExamRange != Exam.Range.Less) ?
			...
			: dan_C.GetAmountToPercent() switch {
				...
				_ => ((amountMax < 0) ? 0 : 100 * amountRemainMax / amountMax) switch {
					< 5 => (dan_C.GetExamStatus() == Exam.Status.Better_Success) ? Exam.ReachStatus.Nearer_Better_Success : Exam.ReachStatus.Nearer_Success,
