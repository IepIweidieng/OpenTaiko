...
//		public bool bBASS演奏で直前のBASSを消音する;
	...
	private void t入力_V4(string strInput, int difficulty) {
		if (!String.IsNullOrEmpty(strInput)) //空なら通さない
		{
			...
			// check difficulty availability, prepend global course, and fix availability for song select
			for (int i = 0; i < (int)Difficulty.Total; i++) {
				if (this.b譜面が存在する[i]) {
					n譜面数++;
					// prepend global course to local course
					if (hasGlobalCourse)
						strSplitした譜面[i] = globalCourse + strSplitした譜面[i];
