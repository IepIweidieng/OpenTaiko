...
internal class CTja : CActivity {
			...
			if (this.bLoadChart) {
				...
				foreach (CChip chip in this.listChip) {
					...
					if (this.COMPAT is not (ETjaCompat.TJAP3 or ETjaCompat.OOS)) {
						bpmPoint = CStagePlayScreenCommon.GetNowPBPMPoint(this, chip.dbSoundTimems, chip.nBranch, useLastDelay: true);
						// do not use earlier or cross-branch bpm points
						if (chip.bpmPoint != null && bpmPoint.bpm_change_course == chip.nBranch && bpmPoint.nInternalNumber < chip.bpmPoint.nInternalNumber)
					...
					for (int ib = 0; ib < 3; ++ib) {
						...
						for (CBPM? bpmChange = this.listBPM[ib]; (bpmChange = bpmChange.next_bpm_change) != null;) {
							...
							if (bpmChange.bpm_change_scroll != 0) {
								...
								th16BeatDriftX = ((long)(dPxJiro1X) - dPxJiro1X) / bpmChange.bpm_change_scroll / pxTh16Beats_Jiro1NoteDist2;
							}
							if (bpmChange.bpm_change_scroll_y != 0) {
								var dPxJiro1Y = bpmChange.bpm_change_scroll_y * dPxJiro1Hs1;
								th16BeatDriftY = ((long)(dPxJiro1Y) - dPxJiro1Y) / bpmChange.bpm_change_scroll_y / pxTh16Beats_Jiro1NoteDist2;
							}

							for (int ibp = bpmChange.nInternalNumber; ibp < this.listBPM.Count; ++ibp) {
	...
	private void ParseCommand(string command, string argument, string argumentFull) {
		if (command == "#START") {
			...
			if (this.COMPAT is ETjaCompat.Jiro1) {
				...
				this.msLastBpmChangeTime = this.dbNowTime = (long)(this.msLastBpmChangeTime + (this.dbNowTime - this.msLastBpmChangeTime));
				this.isAfterLastBpmPoint = true;
			}

			this.ForEachCurrentBranch(branch => {
				var bpmPoint = this.SetBPMPointAtDefCursor(branch, EBPMPointType.Bpm);
			...
			this.ForEachCurrentBranch(branch => this.SetBPMPointAtDefCursor(branch, EBPMPointType.Scroll));
			...
			this.ForEachCurrentBranch(branch => this.SetBPMPointAtDefCursor(branch, EBPMPointType.Measure));
			...
			if (this.COMPAT is ETjaCompat.TJAP3 or ETjaCompat.OOS || nDELAY < 0) {
				this.ForEachCurrentBranch(branch => this.SetBPMPointAtDefCursor(branch, EBPMPointType.DelayEnd));
				this.dbNowTime += nDELAY;
				this.dbNowBMScollTime += nDELAY * this.dbNowBPM / 15000;
			} else if (nDELAY > 0) {
				this.ForEachCurrentBranch(branch => this.SetBPMPointAtDefCursor(branch, EBPMPointType.Delay, nDELAY));
				this.dbNowTime += nDELAY;
				this.ForEachCurrentBranch(branch => this.SetBPMPointAtDefCursor(branch, EBPMPointType.DelayEnd));
			}
		...
		} else if (command == "#LYRIC" && !usingLyricsFile && OpenTaiko.ConfigIni.nPlayerCount < 4) // Do not parse LYRIC tags if a lyric file is already loaded
		{
			...
			this.ForEachCurrentBranch(branch => this.SetBPMPointAtDefCursor(branch, EBPMPointType.ScrollMode));
	...
	private CBPM SetBPMPointAtDefCursor(ECourse branch, EBPMPointType pointType, double msDelayDuration = 0) {
		...
		if (this.listBPM.Count > 0) {
			...
			if (branch == lastBPMPoint.bpm_change_course
				...
				this.listBPM.RemoveAt(this.listBPM.Count - 1); // update last BPM point
				...
				for (int n = 0; n < InputText.Length; n++) {
					...
					if (inputChar == ",") {
						...
						if (this.COMPAT is ETjaCompat.Jiro1 && this.isBpmChangedMeasure) {
							...
							this.isAfterLastBpmPoint = true;
							// Sync BPM Point
							this.ForEachCurrentBranch(branch => this.SetBPMPointAtDefCursor(branch, EBPMPointType.BpmMeasEnd));
						}
						this.isBpmChangedMeasure = false;
						return;
					}
	...
	private void GotoNextDiv(int nDivs) {
		if (this.isBpmChangeInsertedBeforeDiv) {
			...
			this.ForEachCurrentBranch(branch => this.SetBPMPointAtDefCursor(branch, EBPMPointType.BpmAtDiv));
			this.isBpmChangeInsertedBeforeDiv = false;
			this.dbLastDivBPM = this.dbNowBPM;
		}
		...
		this.dbNowBMScollTime += (((this.fNow_Measure_s / this.fNow_Measure_m)) * (16.0 / (double)nDivs));
		this.isAfterLastBpmPoint = true;
	}
	...
	public void UpdateScrolledChipPosition(CChip chip, CBPM nowBpmPoint, double msTjaNowTime, double th16NowBeatX, double th16NowBeatY, double scrollRate) {
		...
		if (NotesManager.IsHittableNote(chip) && msTjaNowTime < velocityRefChip.nSoundTimems - velocityRefChip.msMoveOffset) {
			...
			if (this.COMPAT is ETjaCompat.TMG) {
