		...
		if (!string.IsNullOrEmpty(str全入力文字列)) {
			...
			if (this.bLoadChart) {
				...
				foreach (CChip chip in this.listChip) {
					...
					double th16_beat = CStage演奏画面共通.GetNowPBMTime(bpmPoint, chip.db発声時刻ms, this.COMPAT);
					...
					switch (ch) {
						...
						case 0x08:  // 拡張BPM
						{
								if (this.isOFFSET_Negative)
									chip.n発声時刻ms += this.msOFFSET_Abs;
								if (this.COMPAT is ETjaCompat.TJAP3 or ETjaCompat.OOS && this.listBPM.TryGetValue(chip.n整数値_内部番号, out CBPM cBPM)) {
									bpm = cBPM.dbBPM値;
									this.dbNowBPM = bpm;
								}
								continue;
