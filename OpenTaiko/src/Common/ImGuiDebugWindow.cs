				...
				case CStage.EStage.Game:
					...
					for (int i = 0; i < OpenTaiko.ConfigIni.nPlayerCount; i++) {
						if (ImGui.TreeNodeEx($"Player {i + 1}###GAME_CHART_{i}", ImGuiTreeNodeFlags.Framed)) {
							...
							ImGui.Indent();
							ImGui.Text("Normal: " + dtx.nNotes_Initial_Common.Sum() + dtx.nNotes_Branch[0].Sum() +
									   " / Expert: " + dtx.nNotes_Initial_Common.Sum() + dtx.nNotes_Branch[1].Sum() +
									   " / Master: " + dtx.nNotes_Initial_Common.Sum() + dtx.nNotes_Branch[2].Sum());
							ImGui.Unindent();
